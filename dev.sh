...
case "$term_period" in 
	...
	d|day|Day) cp /home/*/logs/*-${month}-${year}.gz $temp_directory/default ;;
	m|month|Month) cp /home/*/logs/*-${month}-${year}.gz $temp_directory/default ;;
	y|year|Year) cp /home/*/logs/*-${year}.gz $temp_directory/default ;;
	*) ;;
esac
# Iterate through the gzipped log files in $base_directory
echo "[INFO] Entering for-loop at $(date +"%Y-%m-%d_%H:%M:%S")"
for zipped_log in $temp_directory/default/*.gz; do
